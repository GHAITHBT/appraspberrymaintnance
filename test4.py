...
def monitor_working_state():
    global downtime_start, last_working_press
    while True:
        try:
    ...
    try:
        ...
        for pin in [BUTTON_DOWNTIME, BUTTON_MAINTENANCE, BUTTON_EMERGENCY, BUTTON_WORKING]:
            try:
                ...
                logger.warning(f"Error removing event detection for pin {pin}: {remove_error}")
        
        # Add event detection with callbacks
        # Use FALLING edge to detect button press
